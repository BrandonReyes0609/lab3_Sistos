...
// ------------------------- Verificar Todos los Subcuadros con mejoras -------------------------
int verificar_todos_los_subcuadros() {
    int valido = 1;

    omp_set_num_threads(4);   // Usar 4 hilos para paralelismo
    omp_set_nested(1);        // Permitir regiones paralelas anidadas

    #pragma omp parallel for collapse(2) schedule(dynamic) shared(valido)
    ...
    if (valido) {
        printf("✔ Todos los subcuadros son válidos (OpenMP + dynamic scheduling)\n");
    }
    return valido;
}
